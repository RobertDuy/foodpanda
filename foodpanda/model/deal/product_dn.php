...
class ModelDealProductDn extends Model{
    public function insertOrUpdate($data){
        ...
        if(isset($data['link'])){
            ...
            if(isset($queryRow->rows)){
                $product_dn_id = (int)$queryRow->rows['product_dn_id'];
                $customer_dn = $this->model_deal_customer_dn->getCustomerDn($data);
                // update product_ dn
                $this->db->query("UPDATE `". DB_PREFIX ."product_dn` SET number_dn = number_dn + 1 WHERE product_dn_id = ". $product_dn_id);
                ...
                $this->model_deal_customer_dn->insertOrUpdateCusomerDn($data, $product_dn_id);
            }
        }
    }
