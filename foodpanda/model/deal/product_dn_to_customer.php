...
class ModelDealProductDnToCustomer extends Model{
    public function updateSoluong($product_dn_id, $customer_dn_id, $soluong){
        $this->db->query("UPDATE `". DB_PREFIX ."product_dn_to_customer` SET soluong = soluong + ". (int)$soluong . " WHERE product_dn_id = ". (int)$product_dn_id . " AND customer_dn_id = ". (int)$customer_dn_id);
    }
    ...
    public function insertOrUpdate($product_dn_id, $customer_dn_id, $soluong){
        ...
        if(isset($query->row['product_dn_to_customer_id'])){
