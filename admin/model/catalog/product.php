...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
        ...
        if(isset($data['comment'])){
            ...
            noidung = '" . $this->db->escape(html_entity_decode($data['comment'], ENT_QUOTES, 'UTF-8')) ."',
            date_comment = NOW()");
        }
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "', itinerary = '" . $this->db->escape($value['itinerary']) . "', term_condition = '" . $this->db->escape($value['term_condition']) . "', tag = '" . $this->db->escape($value['tag']) . "'");
		}
	...
	public function editProduct($product_id, $data) {
        ...
        if(isset($data['comment'])){
            $this->db->query("UPDATE " . DB_PREFIX . "comment SET
                noidung = '" . $this->db->escape(html_entity_decode($data['noidung'], ENT_QUOTES, 'UTF-8')) . "',
                date_comment = NOW()
            WHERE product_id = '" . (int)$product_id . "'");
        }
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *,
		            ...
		            LEFT JOIN " . DB_PREFIX . "comment c ON (c.product_id = pd.product_id)
